  ...
  return (
    <div className="border-2 border-green-600 p-5 rounded-xl flex flex-col md:flex-row md:items-start md:justify-between text-green-700 space-y-4 md:space-y-0">
      ...
      <div className="flex flex-col md:flex-col items-start md:items-end gap-2 md:ml-6 mt-2 md:mt-0">
        {errorMessage && (
          <div className="text-sm text-red-600 font-medium">⚠️ {errorMessage}</div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-2">
          <button
            ...
            disabled={isLoading}
            className="inline-flex items-center justify-center rounded-md bg-green-600 hover:bg-green-700 text-white px-4 py-2 text-sm font-medium transition cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isLoading ? "Completing..." : "Finish Flight & Complete Contract"}
          </button>
